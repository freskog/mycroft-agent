...
The defining constraint, which explains most design decisions: **the agent runs in a sandbox; credentials and authoritative state do not.** The agent can only *propose* state changes through a CLI that calls a trusted sidecar over localhost HTTP; humans/approval flows finalize. Read `docs/architecture.md` before making structural changes — it documents the *why* behind the boundaries below.
...
- **Gate by risk, not uniformly (HITL).** Memory/entities/relationships are **gateless** — written `accepted` on write, made safe by immutability + provenance + reversibility (`reject`/`archive`/`supersede`); there is no `accept`/`pending`/`accept-all` (removed). **Goals and outside-effect actions are hard-gated** through the approval mechanism: the agent only `request`s; a human decides; person-service executes (`performApproved`). Don't add a direct goal-create or a way for the agent to approve/execute.
